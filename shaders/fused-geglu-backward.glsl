...
float gelu_grad(float x) {
    ...
    float tanh_z = tanh(z);
    float sech_sq = 1.0 / (cosh(z) * cosh(z));
    float dz_dx = sqrt_2_over_pi * (1.0 + 3.0 * coeff * x * x);
    return 0.5 * (1.0 + tanh_z + x * sech_sq * dz_dx);
}
...
void main() {
    ...
    if (pass_type == 0) {
        ...
        for (uint h = 0; h < hidden_dim; h++) {
            ...
            // GeGLU: output = gate * GELU(up)
            float grad_gate = grad_out * gelu(up);
            float grad_up = grad_out * gate * gelu_grad(up);
