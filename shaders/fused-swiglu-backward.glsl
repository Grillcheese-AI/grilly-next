...
// Gate pre-activation (batch_seq, hidden_dim) - computed in forward
layout(set = 0, binding = 4) readonly buffer GateCache {
    float gate_cache[];
};

// Up pre-activation (batch_seq, hidden_dim) - computed in forward
...
void main() {
    ...
    if (pass_type == 0) {
        ...
        // grad_input[b, k] = sum_h (grad_gate[b,h] * W_gate[h,k] + grad_up[b,h] * W_up[h,k])
        ...
        for (uint h = 0; h < hidden_dim; h++) {
            ...
            float grad_gate = grad_out * silu(up);
            float grad_up = grad_out * gate * silu_grad(up);

            // Accumulate gradient through both paths
        ...
        // grad_W_gate[h, k] = sum_b (grad_gate[b,h] * input[b,k])
        // grad_W_up[h, k] = sum_b (grad_up[b,h] * input[b,k])
