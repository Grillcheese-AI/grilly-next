...
/*
 * Fused Linear + ReLU Shader
 *
 * Combines linear transformation and ReLU activation in a single pass.
...
void main() {
    ...
    // Step 2: Apply ReLU activation (fused)
    float relu_out = max(0.0, sum);

    // Write final result
    uint out_idx = row * output_dim + col;
    output_data[out_idx] = relu_out;
}
