...
def main():
    ...
    # --- 2. Format with Qwen's Chat Template & STRICT TOKEN TRUNCATION ---
    print("Applying Qwen chat template and strictly truncating to 3800 tokens...")
    formatted_prompts = []
    for raw_text in raw_prompts: 
        ...
        # FIX: Dynamically truncate the string if it exceeds 3800 tokens
        # We encode it, truncate the array, and decode it back to text safely.
        tokenized_prompt = tokenizer.encode(raw_text, truncation=True, max_length=3800)
        safe_raw_text = tokenizer.decode(tokenized_prompt, skip_special_tokens=True)
    ...
    CHUNK_SIZE = 25000
    # RESUMING EXACTLY WHERE WE CRASHED
    RESUME_INDEX = 150000 
